...
TypeScript is configured with relaxed strictness (`noImplicitAny: false`, `strictNullChecks: false`). This is intentional for this codebase.

## Key Architectural Patterns

### Authentication & RBAC
Auth is managed via `AuthContext` ([AuthContext.tsx](src/contexts/AuthContext.tsx)). Role-based permissions:
...
- `SpedService` - SPED fiscal document processing
- `LLMService` - LLM API interactions
...
2. **AI Library Agents**: Use N8N webhooks via `n8nAgentService.ts`. Configured in [aiAgents.ts](src/config/aiAgents.ts) (50+ agents across 11 themes). Accessed via `AgentChat.tsx`.
...
New features follow a modular structure in `src/features/`:
```
...
Example: `src/features/leads/` implements the leads management feature.

### Routing Structure
Routes are defined in [App.tsx](src/App.tsx):
- `/login` - Public login page
- `/` - Dashboard (protected)
- `/assistants/*` - AI assistant chats and library
- `/documents/*` - Document management (payroll, SPED, cases)
- `/companies/*` - Company management (requires `companies` permission)
- `/admin/*` - User administration (requires `admin` permission)
...
- `chat-completion` - AI chat completions with multi-model support
...
- The codebase is primarily in Portuguese (Brazilian)
