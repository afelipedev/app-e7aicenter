-- Corrige "Precisa de atenção": deixa de usar payload->>'title' (inexistente)
-- com fallback "Menção em Equipes" para qualquer notificação não lida.
-- A faixa passa a listar só o que é do usuário autenticado: menções,
-- cards/quadros atribuídos, respostas, convites e postagens, além de
-- cards atrasados/bloqueados atribuídos a ele.
    ...
    SELECT * FROM (
      SELECT
        ...
        CASE WHEN b.domain = 'operational'
          THEN '/gestao-operacional/quadros/' || b.slug || '?card=' || c.id::text
          ELSE '/documents/cases/quadros/' || b.slug || '?card=' || c.id::text
        END AS href,
      ...
      SELECT
        CASE n.kind
          WHEN 'post_mention' THEN 3
          WHEN 'message_mention' THEN 3
          WHEN 'kanban_comment_mention' THEN 3
          WHEN 'card_member_added' THEN 4
          WHEN 'card_pending_approval' THEN 4
          WHEN 'board_member_added' THEN 5
          WHEN 'post_reply' THEN 5
          WHEN 'team_invite' THEN 6
          ELSE 8
        END,
        n.kind,
        n.id::text,
        CASE n.kind
          WHEN 'post_mention' THEN
            CASE WHEN nullif(n.payload->>'post_title', '') IS NOT NULL
              THEN 'Você foi mencionado na postagem "' || (n.payload->>'post_title') || '"'
              ELSE 'Você foi mencionado em uma postagem' END
          WHEN 'message_mention' THEN
            CASE WHEN nullif(n.payload->>'post_title', '') IS NOT NULL
              THEN 'Você foi mencionado em uma resposta da postagem "' || (n.payload->>'post_title') || '"'
              ELSE 'Você foi mencionado em uma mensagem' END
          WHEN 'kanban_comment_mention' THEN
            CASE WHEN nullif(n.payload->>'card_title', '') IS NOT NULL
              THEN 'Você foi mencionado no card "' || (n.payload->>'card_title') || '"'
              ELSE 'Você foi mencionado em um card' END
          WHEN 'card_member_added' THEN
            CASE WHEN nullif(n.payload->>'card_title', '') IS NOT NULL
              THEN 'Você foi adicionado ao card "' || (n.payload->>'card_title') || '"'
              ELSE 'Você foi adicionado a um card' END
          WHEN 'board_member_added' THEN
            CASE WHEN nullif(n.payload->>'board_title', '') IS NOT NULL
              THEN 'Você foi adicionado ao quadro "' || (n.payload->>'board_title') || '"'
              ELSE 'Você foi adicionado a um quadro' END
          WHEN 'card_pending_approval' THEN
            CASE WHEN nullif(n.payload->>'card_title', '') IS NOT NULL
              THEN 'O card "' || (n.payload->>'card_title') || '" aguarda aprovação'
              ELSE 'Um card aguarda aprovação' END
          WHEN 'post_reply' THEN
            CASE WHEN nullif(n.payload->>'post_title', '') IS NOT NULL
              THEN 'Nova resposta em "' || (n.payload->>'post_title') || '"'
              ELSE 'Nova resposta em uma postagem' END
          WHEN 'team_invite' THEN 'Você foi adicionado a uma equipe'
          ELSE coalesce(n.payload->>'post_title', n.payload->>'card_title', 'Notificação')
        END,
        coalesce(
          CASE WHEN nullif(n.payload->>'channel_name', '') IS NOT NULL
            THEN '#' || (n.payload->>'channel_name') END,
          n.payload->>'board_title',
          ''
        ),
        CASE
          WHEN n.kind IN ('post_mention', 'post_reply', 'message_mention')
               AND nullif(n.payload->>'team_slug', '') IS NOT NULL
               AND nullif(n.payload->>'channel_slug', '') IS NOT NULL
               AND nullif(n.payload->>'post_id', '') IS NOT NULL
            THEN '/teams/' || (n.payload->>'team_slug') || '/' || (n.payload->>'channel_slug') || '/' || (n.payload->>'post_id')
              || CASE WHEN n.kind = 'message_mention' AND nullif(n.payload->>'message_id', '') IS NOT NULL
                   THEN '?messageId=' || (n.payload->>'message_id') ELSE '' END
          WHEN n.kind IN ('kanban_comment_mention', 'card_member_added', 'card_pending_approval')
               AND nullif(n.payload->>'board_slug', '') IS NOT NULL
            THEN CASE WHEN coalesce(b.domain, 'legal') = 'operational'
                   THEN '/gestao-operacional/quadros/' ELSE '/documents/cases/quadros/' END
                 || (n.payload->>'board_slug')
                 || CASE WHEN nullif(n.payload->>'card_id', '') IS NOT NULL
                      THEN '?card=' || (n.payload->>'card_id') ELSE '' END
          WHEN n.kind = 'board_member_added'
               AND nullif(n.payload->>'board_slug', '') IS NOT NULL
            THEN CASE WHEN coalesce(b.domain, 'legal') = 'operational'
                   THEN '/gestao-operacional/quadros/' ELSE '/documents/cases/quadros/' END
                 || (n.payload->>'board_slug')
          WHEN n.kind = 'team_invite' AND nullif(n.payload->>'team_slug', '') IS NOT NULL
            THEN '/teams/' || (n.payload->>'team_slug')
          ELSE '/teams'
        END,
        CASE WHEN n.kind = 'card_pending_approval' THEN 'warning' ELSE 'info' END,
        n.created_at
      FROM notifications n
      LEFT JOIN legal_kanban_boards b ON b.slug = n.payload->>'board_slug'
      WHERE v_uid IS NOT NULL
        AND n.user_id = v_uid
        AND n.read_at IS NULL
        AND n.kind IN (
          'post_mention', 'message_mention', 'kanban_comment_mention',
          'card_member_added', 'card_pending_approval', 'board_member_added',
          'post_reply', 'team_invite'
        )
    ) raw
