...
const ASSISTANT_TYPE = "chat-general";
...
export default function ChatGeneral() {
  ...
  // Sincronizar modelo do chat atual
  useEffect(() => {
  ...
  const callChatCompletion = async (chatId: string, message: string) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await callChatCompletion(currentChat.id, userMessage);
  ...
  const handleModelChange = async (model: LLMModel) => {
    ...
    if (currentChat) {
      try {
        // Atualizar modelo do chat atual
        // Nota: isso pode criar um novo chat ou atualizar o existente dependendo da UX desejada
        // Por enquanto, apenas atualizamos o modelo do chat atual
            ...
            <ScrollArea className="flex-1 min-h-0">
              <div className={cn(
                ...
                {messages.map((message, index) => {
                  // Usar ID da mensagem se disponível, senão usar uma key única baseada no conteúdo e índice
                  const messageKey = message.id || `${message.role}-${index}-${message.content.slice(0, 30)}-${message.createdAt || index}`;
                  // Buscar metadata da mensagem (se disponível)
                  const metadata = (currentChat as any)?.messagesMetadata?.[index];
                  return (
                    <ChatMessage
                      key={messageKey}
                      message={message}
