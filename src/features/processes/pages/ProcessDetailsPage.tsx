...
type ProcessDetailsTab = "movements" | "information" | "agent";
...
function buildMovementGroups(movements: ProcessDetail["movements"]) {
  const groups = new Map<
    string,
    { key: string; label: string; sortValue: number; movements: ProcessDetail["movements"] }
  >();
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card className="rounded-[24px] border-border/70 bg-card/90 p-6 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.55)]">
          <div className="flex items-start gap-4">
            ...
            <DetailField label="Formato" value={process.formato?.nome ?? "Não informado"} />
          </div>
        </Card>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as ProcessDetailsTab)} className="space-y-6">
        ...
        <TabsContent value="movements" className="space-y-4">
          {movementGroups.length === 0 ? (
            <Card className="rounded-[24px] border-dashed border-border/70 p-8 text-center text-sm text-muted-foreground">
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              <Card className="rounded-[24px] border-border/70 bg-card/95 p-6 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.55)]">
                <div className="grid gap-4">
                  <DetailField label="Número CNJ" value={process.cnj} />
                  <DetailField label="Tribunal" value={process.tribunal} />
                  <DetailField label="Grau do processo" value={`${process.grade}${process.grau ? ` (${process.grau})` : ""}`} />
                  <DetailField label="Órgão julgador" value={process.orgaoJulgador} />
                  <DetailField label="Ajuizado em" value={process.distributedAt} />
                </div>
              </Card>
              ...
              <Card className="rounded-[24px] border-border/70 bg-card/95 p-6 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.55)]">
                <div className="grid gap-4">
                  <DetailField label="Classe processual" value={process.classProcessual} />
                  <DetailField label="Assuntos" value={process.assuntos.join(", ") || "Não informado"} />
                  <DetailField label="Formato" value={process.formato?.nome ?? "Não informado"} />
                  <DetailField label="Sistema" value={process.sistema?.nome ?? "Não informado"} />
                  <DetailField
                    label="Nível de sigilo"
                    value={process.nivelSigilo != null ? String(process.nivelSigilo) : "Não informado"}
                  />
                </div>
              </Card>
            </div>
              ...
              <div className="space-y-5">
                ...
                <p className="text-sm leading-7 text-white/80">
                  {process.aiDisclaimer ||
                    "O E7 Agente Processual consolida os metadados processuais do DataJud para apoiar leitura e tomada de decisão."}
                </p>
